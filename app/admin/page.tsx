...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-2xl bg-white p-5" style={{ border: "1px solid var(--border)" }}>
      <div className="font-heading text-3xl" style={{ fontWeight: 600 }}>{value}</div>
...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl mb-5" style={{ fontWeight: 600 }}>Visão geral</h1>
      ...
      <h2 className="font-heading text-xl mt-10 mb-3" style={{ fontWeight: 600 }}>Avaliações para revisar ({pendingReviews.length})</h2>
      {pendingReviews.length === 0 ? (
        <p className="text-sm" style={{ color: "var(--muted)" }}>Nenhuma avaliação pendente.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {pendingReviews.map((r: any) => (
            <div key={r.id} className="rounded-2xl bg-white p-4 flex items-start justify-between gap-4" style={{ border: "1px solid var(--border)" }}>
              <div>
