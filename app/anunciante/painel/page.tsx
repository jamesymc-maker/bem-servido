...
export default async function AdvertiserHome() {
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl mb-1" style={{ fontWeight: 600 }}>Olá, {adv?.company_name || "anunciante"}</h1>
      <p className="text-sm mb-6" style={{ color: "var(--muted)" }}>
        {hasActivePlan ? "Conta ativa. Gerencie os seus anúncios abaixo." : "Escolha um plano para activar os seus anúncios."}
      </p>
      {!hasActivePlan && (
        <div className="rounded-2xl p-5 mb-6 flex items-center justify-between gap-4" style={{ background: "rgba(226,100,63,.08)", border: "1px solid var(--pink)" }}>
          <p className="text-sm font-medium" style={{ color: "var(--pink)" }}>Nenhum plano activo. Os seus anúncios não estão a ser exibidos.</p>
          <Link href="/anunciante/painel/plano" className="shrink-0 rounded-full px-4 py-2 text-sm font-semibold text-white" style={{ background: "var(--pink)" }}>Escolher plano</Link>
        </div>
      )}
      <h2 className="font-heading text-xl mb-3" style={{ fontWeight: 600 }}>Dados da empresa</h2>
      <form action={updateAdvertiserProfile} className="flex flex-col gap-4">
        ...
        <button className="self-start rounded-full px-5 py-2.5 font-semibold text-white text-sm" style={{ background: "var(--teal)" }}>Guardar dados</button>
      </form>
    </div>
  );
}
