...
function BrowserFrame({ children, label }: { children: React.ReactNode; label: string }) {
  return (
    <div className="rounded-2xl overflow-hidden bg-white" style={{ border: "1px solid var(--border)", boxShadow: "0 24px 50px -32px rgba(14,91,78,.35)" }}>
      <div className="flex items-center gap-1.5 px-3 py-2.5" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        <span className="ml-2 text-[11px] truncate" style={{ color: "var(--muted)" }}>{label}</span>
      </div>
...
function AdMock({ tall = false }: { tall?: boolean }) {
  return (
    <div className="relative rounded-xl flex items-center justify-center text-center"
      style={{ aspectRatio: tall ? "5 / 1.4" : "5 / 1", background: "linear-gradient(120deg, var(--teal), #1c7a69)" }}>
      <div>
        <div className="text-[11px] font-bold uppercase tracking-wide" style={{ color: "rgba(255,255,255,.7)" }}>O seu anúncio aqui</div>
        <div className="font-heading text-white text-sm sm:text-base" style={{ fontWeight: 600 }}>A sua marca em destaque</div>
      </div>
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: "radial-gradient(120% 90% at 80% -10%, rgba(28,122,105,.22), transparent 55%), radial-gradient(90% 80% at -10% 20%, rgba(226,100,63,.16), transparent 55%), var(--white)" }} />
        <div className="relative max-w-6xl mx-auto px-5 pt-16 pb-12 md:pt-20 grid md:grid-cols-[1.05fr_.95fr] gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[13px] font-medium mb-6"
              style={{ background: "#fff", border: "1px solid var(--border)", color: "var(--teal)" }}>
              <MapPin size={14} /> Para empresas · {loc.name}
            </div>
            <h1 className="font-heading leading-[1.04] tracking-tight" style={{ fontSize: "clamp(2.4rem,5.5vw,4rem)", fontWeight: 600 }}>
              Seja visto por quem<br /><span style={{ color: "var(--pink)" }}>chega em {loc.name}</span>
            </h1>
            <p className="mt-6 text-lg max-w-md" style={{ color: "var(--muted)" }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/anunciante/criar-conta" className="inline-flex items-center gap-2 rounded-full px-6 py-3.5 font-semibold text-white transition active:scale-95" style={{ background: "var(--teal)" }}>
                Começar agora <ArrowRight size={17} />
              </Link>
              <Link href="#planos" className="inline-flex items-center gap-2 rounded-full px-6 py-3.5 font-semibold transition active:scale-95" style={{ border: "1px solid var(--border)", color: "var(--navy)" }}>
                Ver planos e preços
              </Link>
            </div>
            <div className="mt-8 flex flex-wrap gap-x-8 gap-y-3">
              {stats.slice(0, 3).map(([n, l]) => (
                <div key={l}>
                  <div className="font-heading text-2xl" style={{ fontWeight: 600 }}>{n}</div>
          ...
          <div className="hidden sm:block">
            <BrowserFrame label="daquii.com">
              ...
              <div className="grid grid-cols-3 gap-2.5 mt-3">
                {[0, 1, 2].map((i) => (
                  <div key={i} className="rounded-xl p-2.5" style={{ border: "1px solid var(--border)" }}>
      ...
      <section className="max-w-5xl mx-auto px-5 py-14">
        <div className="text-center max-w-2xl mx-auto mb-10">
          <h2 className="font-heading text-3xl" style={{ fontWeight: 600 }}>Um público que está prestes a gastar na ilha</h2>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {AUDIENCE.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="rounded-3xl p-7 bg-white" style={{ border: "1px solid var(--border)" }}>
      ...
      <section className="py-14" style={{ background: "var(--sand)" }}>
        <div className="max-w-6xl mx-auto px-5">
          <div className="text-center max-w-2xl mx-auto mb-10">
            <div className="inline-flex items-center gap-2 text-sm font-semibold mb-3" style={{ color: "var(--teal)" }}>
              <Sparkles size={15} /> Onde a sua marca aparece
            </div>
            <h2 className="font-heading text-3xl" style={{ fontWeight: 600 }}>Espaços nobres por todo o site</h2>
            <p className="mt-3 text-lg" style={{ color: "var(--muted)" }}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-3 px-1">
                ...
                <span className="text-xs ml-auto" style={{ color: "var(--muted)" }}>1200×240px</span>
              </div>
            </div>
            ...
            <div>
              <BrowserFrame label="Categoria · Chefs privativos">
                ...
                <div className="rounded-lg mb-3 flex items-center gap-2 p-2" style={{ border: "1px solid var(--border)" }}>
                ...
                <AdMock />
                <div className="rounded-lg mt-3 flex items-center gap-2 p-2" style={{ border: "1px solid var(--border)" }}>
                  <div className="w-9 h-9 rounded-lg shrink-0" style={{ background: "rgba(0,0,0,.06)" }} />
                  <div className="flex-1">{skel("65%", 6)}<div className="mt-1.5">{skel("40%", 6)}</div></div>
                </div>
              </BrowserFrame>
              <div className="flex items-center gap-2 mt-3 px-1">
                <LayoutGrid size={16} style={{ color: "var(--teal)" }} />
                <span className="text-sm font-semibold">Banner de categoria</span>
                <span className="text-xs ml-auto" style={{ color: "var(--muted)" }}>1000×200px</span>
              </div>
            </div>
            ...
            <div>
              <BrowserFrame label="Perfil do profissional">
                ...
                <div className="flex items-center justify-center gap-2 mt-3 pt-3" style={{ borderTop: "1px solid var(--border)" }}>
                  <BadgeCheck size={13} style={{ color: "var(--teal)" }} />
                  <span className="text-[10px]" style={{ color: "var(--muted)" }}>Parceiro Oficial · logótipo no rodapé</span>
                </div>
              </BrowserFrame>
              <div className="flex items-center gap-2 mt-3 px-1">
      ...
      <section id="planos" className="max-w-5xl mx-auto px-5 py-16 scroll-mt-20">
        <div className="text-center max-w-2xl mx-auto mb-10">
          <h2 className="font-heading text-3xl" style={{ fontWeight: 600 }}>Escolha o seu alcance</h2>
          <p className="mt-3 text-lg" style={{ color: "var(--muted)" }}>Planos mensais, sem fidelização. Cancele quando quiser.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-5 items-stretch">
          {AD_PLANS.map((pl) => (
            <div key={pl.key} className="rounded-3xl p-7 flex flex-col"
              style={{ background: pl.accent ? "var(--teal)" : "#fff", color: pl.accent ? "#fff" : "var(--navy)",
                border: pl.accent ? "none" : "1px solid var(--border)", transform: pl.accent ? "scale(1.03)" : "none",
                boxShadow: pl.accent ? "0 30px 60px -30px rgba(14,91,78,.6)" : "none" }}>
              {pl.accent && <span className="self-start text-[11px] font-bold uppercase tracking-wide rounded-full px-2.5 py-1 mb-3" style={{ background: "var(--pink)", color: "#fff" }}>Mais escolhido</span>}
              <h3 className="font-heading text-2xl" style={{ fontWeight: 600 }}>{pl.name}</h3>
              <p className="text-sm mt-1" style={{ color: pl.accent ? "rgba(255,255,255,.8)" : "var(--muted)" }}>{pl.blurb}</p>
              <div className="mt-4 mb-2">
                <span className="font-heading text-4xl" style={{ fontWeight: 600 }}>{brl(pl.price)}</span>
                <span className="text-sm" style={{ color: pl.accent ? "rgba(255,255,255,.7)" : "var(--muted)" }}> / mês</span>
              </div>
      ...
      <section className="py-14" style={{ background: "var(--sand)" }}>
        <div className="max-w-5xl mx-auto px-5">
          <div className="text-center max-w-2xl mx-auto mb-9">
            <h2 className="font-heading text-3xl" style={{ fontWeight: 600 }}>Conte a sua história</h2>
            <p className="mt-3 text-lg" style={{ color: "var(--muted)" }}>
              Planos Destaque e Parceiro incluem posts patrocinados no nosso blog — conteúdo que aparece em buscas e gera confiança.
            </p>
          </div>
          <article className="max-w-2xl mx-auto rounded-3xl overflow-hidden bg-white" style={{ border: "1px solid var(--border)" }}>
            <div className="relative">
              ...
              />
              <span className="absolute top-3 left-3 inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide rounded-full px-2.5 py-1" style={{ background: "var(--pink)", color: "#fff" }}>
                <Sparkles size={12} /> Post patrocinado
              </span>
            </div>
            <div className="p-6">
              <div className="flex items-center gap-2 text-xs mb-2" style={{ color: "var(--muted)" }}>
                <span className="inline-flex items-center gap-1"><BadgeCheck size={13} style={{ color: "var(--teal)" }} /> Por uma marca parceira</span>
                <span>·</span>
                <span>Leitura de 4 min</span>
              </div>
              <h3 className="font-heading text-2xl leading-snug mb-2" style={{ fontWeight: 600 }}>
              ...
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold mt-4" style={{ color: "var(--teal)" }}>
      ...
      <section className="max-w-5xl mx-auto px-5 py-16">
        <div className="rounded-3xl p-8 md:p-10" style={{ background: "var(--teal)" }}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {stats.map(([n, l]) => (
              <div key={l}>
                <div className="font-heading text-white" style={{ fontSize: "clamp(2rem,4vw,2.8rem)", fontWeight: 600 }}>{n}</div>
      ...
      <section className="max-w-3xl mx-auto px-5 pb-16">
        <h2 className="font-heading text-3xl text-center mb-8" style={{ fontWeight: 600 }}>Perguntas frequentes</h2>
        <div className="flex flex-col gap-3">
          {FAQ.map((item) => (
            <details key={item.q} className="group rounded-2xl bg-white p-5" style={{ border: "1px solid var(--border)" }}>
              <summary className="flex items-center justify-between cursor-pointer list-none font-semibold">
                {item.q}
                <span className="ml-3 shrink-0 transition group-open:rotate-45" style={{ color: "var(--teal)" }}>
      ...
      {/* ---------- FINAL CTA ---------- */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: "radial-gradient(120% 100% at 50% -20%, rgba(14,91,78,.18), transparent 60%), var(--white)" }} />
        <div className="relative max-w-3xl mx-auto px-5 py-16 text-center">
          <h2 className="font-heading" style={{ fontSize: "clamp(2rem,4.5vw,3rem)", fontWeight: 600 }}>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link href="/anunciante/criar-conta" className="inline-flex items-center gap-2 rounded-full px-7 py-4 font-semibold text-white transition active:scale-95" style={{ background: "var(--teal)" }}>
              Criar conta de anunciante <ArrowRight size={17} />
            </Link>
            <Link href="/anunciante/entrar" className="inline-flex items-center rounded-full px-7 py-4 font-semibold transition active:scale-95" style={{ border: "1px solid var(--border)", color: "var(--navy)" }}>
              Já tenho conta
            </Link>
          </div>
        </div>
