...
function Item({ done, label, href }: { done: boolean; label: string; href: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 rounded-2xl bg-white p-4 transition hover:shadow-sm" style={{ border: "1px solid var(--border)" }}>
      {done ? <Check size={20} style={{ color: "var(--teal)" }} /> : <Circle size={20} style={{ color: "var(--border)" }} />}
      <span className="font-medium" style={{ color: done ? "var(--muted)" : "var(--navy)" }}>{label}</span>
    </Link>
  );
}
...
export default async function PainelHome() {
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl mb-1" style={{ fontWeight: 600 }}>Olá, {(p?.name || "").split(" ")[0] || "profissional"}</h1>
      ...
      {trialDaysLeft !== null && !trialExpired && sub?.status !== "active" && (
        <div className="rounded-2xl p-4 mb-4 flex items-center justify-between gap-3" style={{ background: "rgba(14,91,78,.07)", border: "1px solid var(--border)" }}>
          <p className="text-sm"><span className="font-semibold" style={{ color: "var(--teal)" }}>Período gratuito:</span> {trialDaysLeft} dias restantes</p>
        </div>
      )}
      {trialExpired && (
        <div className="rounded-2xl p-4 mb-4 flex items-center justify-between gap-4" style={{ background: "rgba(226,100,63,.08)", border: "1px solid var(--pink)" }}>
          <p className="text-sm font-medium" style={{ color: "var(--pink)" }}>O seu período gratuito terminou. Escolha um plano para manter o perfil activo.</p>
          <a href="/painel/plano" className="shrink-0 rounded-full px-4 py-2 text-sm font-semibold text-white" style={{ background: "var(--pink)" }}>Escolher plano</a>
        </div>
      )}
