...
export function AdUploader({ advertiserId, adId, initialImage }: { advertiserId: string; adId?: string; initialImage?: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-xl p-3.5" style={{ background: "var(--sand)", border: "1px solid var(--border)" }}>
        <div className="flex items-center gap-2 mb-2 text-sm font-semibold" style={{ color: "var(--navy)" }}>
          <Info size={15} style={{ color: "var(--teal)" }} /> Especificações da imagem
        </div>
        <div className="grid sm:grid-cols-2 gap-2 mb-2">
          {PLACEMENTS.map((p) => (
            <div key={p.label} className="rounded-lg bg-white px-3 py-2" style={{ border: "1px solid var(--border)" }}>
              <div className="text-xs font-semibold" style={{ color: "var(--navy)" }}>{p.label}</div>
              <div className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>{p.dims} · proporção {p.ratio}</div>
            </div>
          ))}
        </div>
        <ul className="flex flex-wrap gap-x-4 gap-y-1 text-xs" style={{ color: "var(--muted)" }}>
          <li className="flex items-center gap-1"><Check size={12} style={{ color: "var(--teal)" }} /> Formatos: JPG, PNG ou WebP</li>
          <li className="flex items-center gap-1"><Check size={12} style={{ color: "var(--teal)" }} /> Tamanho máximo: 2 MB</li>
          <li className="flex items-center gap-1"><Check size={12} style={{ color: "var(--teal)" }} /> Banner horizontal (formato 5:1)</li>
        </ul>
      </div>
      ...
      <div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)", background: "var(--white)" }}>
          {/* fake browser chrome */}
          <div className="flex items-center gap-1.5 px-3 py-2" style={{ borderBottom: "1px solid var(--border)", background: "#fff" }}>
            <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#ff5f57" }} />
            <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#febc2e" }} />
            <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#28c840" }} />
            <span className="ml-2 text-[10px] truncate" style={{ color: "var(--muted)" }}>daquii.com</span>
          </div>
          <div className="p-3">
            {/* page skeleton rows */}
            ...
            <div className="relative rounded-lg overflow-hidden flex items-center justify-center"
              style={{ aspectRatio: "5 / 1", border: img ? "none" : "2px dashed var(--border)", background: img ? "transparent" : "rgba(14,91,78,.04)" }}>
              {img ? (
                ...
                <div className="flex flex-col items-center gap-1 text-center px-2">
                  ...
                  <span className="text-[11px]" style={{ color: "var(--muted)" }}>O seu banner aparece aqui</span>
                </div>
              )}
              {img && (
      ...
      {err && <p className="text-xs" style={{ color: "var(--pink)" }}>{err}</p>}
    </div>
  );
}
...
export function LogoUploader({ advertiserId, initialLogo }: { advertiserId: string; initialLogo?: string | null }) {
  ...
  return (
    <div className="flex items-center gap-4">
      {logo && <img src={logo} alt="Logo" className="w-16 h-16 object-contain rounded-xl" style={{ border: "1px solid var(--border)" }} referrerPolicy="no-referrer" />}
      <div>
