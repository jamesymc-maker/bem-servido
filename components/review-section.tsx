  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} size={size} strokeWidth={0} fill={i < Math.round(n) ? "var(--coral)" : "var(--sand-deep)"} />
      ))}
    </div>
  );
}
  ...
  return (
    <section className="mt-10">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <h2 className="serif text-2xl flex items-center gap-3" style={{ fontWeight: 600 }}>
          {t("reviews.title")}
          {summary.count > 0 && (
            <span className="inline-flex items-center gap-2 text-base font-sans" style={{ color: "var(--ink-soft)" }}>
              <StarsRow n={summary.avg} />
              <span className="font-semibold" style={{ color: "var(--ink)" }}>{summary.avg.toFixed(1)}</span>
        ...
        {!openForm && state !== "done" && (
          <button onClick={() => setOpenForm(true)}
            ...
            style={{ background: "var(--sea)", color: "#fff" }}>{t("reviews.leave")}</button>
        )}
      </div>

      <p className="text-[13px] mt-1" style={{ color: "var(--ink-soft)" }}>{t("reviews.moderated")}</p>

      {state === "done" ? (
        <div className="mt-5 rounded-2xl px-5 py-4 text-sm" style={{ background: "rgba(14,91,78,.08)", color: "var(--sea)" }}>
          {t("reviews.thanks")}
        </div>
      ) : openForm ? (
        <div className="mt-5 rounded-2xl bg-white p-5" style={{ border: "1px solid var(--line)" }}>
          ...
          <div className="flex gap-1 mb-4">
            {[1, 2, 3, 4, 5].map((v) => (
              <button key={v} onMouseEnter={() => setHover(v)} onMouseLeave={() => setHover(0)} onClick={() => setRating(v)} aria-label={`${v}`}>
                <Star size={28} strokeWidth={0} fill={(hover || rating) >= v ? "var(--coral)" : "var(--sand-deep)"} />
              </button>
            ))}
          </div>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("reviews.yourName")}
            className="w-full rounded-xl px-4 py-3 mb-3 bg-white outline-none text-[15px]" style={{ border: "1px solid var(--line)" }} />
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder={t("reviews.yourComment")} rows={4}
            className="w-full rounded-xl px-4 py-3 bg-white outline-none text-[15px] resize-none" style={{ border: "1px solid var(--line)" }} />
          {err && <p className="text-sm mt-2" style={{ color: "var(--coral)" }}>{err}</p>}
          <div className="flex gap-2 mt-4">
            <button onClick={submit} disabled={state === "sending"}
              className="rounded-full px-5 py-3 text-sm font-semibold text-white transition active:scale-95 disabled:opacity-60" style={{ background: "var(--sea)" }}>
              {state === "sending" ? t("reviews.sending") : t("reviews.submit")}
            </button>
            <button onClick={() => { setOpenForm(false); setErr(null); }} className="rounded-full px-5 py-3 text-sm font-semibold" style={{ background: "var(--sand)", color: "var(--ink)" }}>
              {t("reviews.cancel")}
            </button>
          </div>
        </div>
      ...
      <div className="mt-6 flex flex-col gap-4">
        {reviews.length === 0 ? (
          <p className="text-[15px]" style={{ color: "var(--ink-soft)" }}>{t("reviews.empty", { name: providerName.split(" ")[0] })}</p>
        ) : (
          reviews.map((r) => (
