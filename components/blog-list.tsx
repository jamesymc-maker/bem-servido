...
export function BlogList({ posts }: { posts: Post[] }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 py-12">
      <div className="mb-9">
        <h1 className="serif text-4xl md:text-5xl" style={{ fontWeight: 600 }}>{t("blog.title")}</h1>
        <p className="text-lg mt-2" style={{ color: "var(--ink-soft)" }}>{t("blog.sub")}</p>
      </div>
      {posts.length === 0 ? (
        <p className="py-16 text-center" style={{ color: "var(--ink-soft)" }}>{t("blog.empty")}</p>
      ) : (
          ...
          {posts.map((p, i) => (
            <Link key={p.slug} href={`/${slug}/blog/${p.slug}`}
              ...
              <div className="p-5 flex flex-col gap-2 flex-1">
                ...
                <p className="text-sm leading-relaxed flex-1" style={{ color: "var(--ink-soft)" }}>{p.description}</p>
                <span className="text-sm font-semibold mt-1" style={{ color: "var(--sea)" }}>{t("blog.readMore")} →</span>
              </div>
            </Link>
          ))}
        </div>
