    ...
    <div className="flex gap-0.5" aria-label={`${n} de 5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} size={15} strokeWidth={0}
          fill={i < n ? "var(--pink)" : "var(--sand-deep)"} />
      ))}
    </div>
  );
}
...
function Card({ tItem, i, provider }: { tItem: Testimonial; i: number; provider: boolean }) {
  return (
    <figure className="rise rounded-3xl bg-white p-6 flex flex-col gap-4 h-full"
      style={{ border: "1px solid var(--border)", animationDelay: `${i * 70}ms` }}>
      <div className="flex items-center justify-between">
        <Stars n={tItem.rating} />
        <Quote size={22} style={{ color: "var(--sand-deep)" }} />
      </div>
      <blockquote className="text-[15px] leading-relaxed flex-1" style={{ color: "var(--navy)" }}>
        “{tItem.quote}”
      </blockquote>
      {provider && tItem.stat && (
        <div className="inline-flex items-baseline gap-1.5 self-start rounded-full px-3 py-1.5"
          ...
          <span className="font-heading font-semibold">{tItem.stat}</span>
          <span className="text-[12px]" style={{ color: "var(--muted)" }}>{tItem.statLabel}</span>
        </div>
      )}
      <figcaption className="flex items-center gap-3 pt-1" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        <div className="mt-3">
          ...
          <div className="text-[13px]" style={{ color: "var(--muted)" }}>{tItem.meta}</div>
        </div>
      </figcaption>
    </figure>
  );
    ...
    <section className="py-16" style={{ background: provider ? "transparent" : "var(--sand)" }}>
      <div className="max-w-6xl mx-auto px-5">
        <div className="text-center max-w-2xl mx-auto mb-9">
          <div className="inline-flex items-center gap-1.5 text-sm font-semibold mb-2" style={{ color: "var(--pink)" }}>
            <Star size={15} fill="var(--pink)" strokeWidth={0} /> {provider ? t("testimonials.providersTag") : t("testimonials.customersTag")}
          </div>
          <h2 className="font-heading text-3xl md:text-4xl" style={{ fontWeight: 600 }}>{title}</h2>
          <p className="text-[15px] mt-2" style={{ color: "var(--muted)" }}>{sub}</p>
        </div>
